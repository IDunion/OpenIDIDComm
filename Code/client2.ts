...
server.post("/didcomm", bodyParser.raw({ type: "text/plain" }), async (req: Request, res: Response) => {
  ...
  if (message.type == "ack_registration") {
    console.log("> Registrierung Bestätigung #"+message.threadId)
    debug(message)
    const {connection_id} = message.data as {connection_id:string}
    
    if (outstanding_registrations[message.threadId!]){
      outstanding_registrations[message.threadId!](connection_id)
    }
  }
...
var outstanding_registrations: Record<string, (val:string) => void> = {}
...
const connection_id_promise = new Promise<string>( async (res, rej) => {
  ...
  agent.sendDIDCommMessage({ messageId: message.id, packedMessage: packed_msg, recipientDidUrl: message.to })
  outstanding_registrations[message.id] = res
  console.log("< Registriere DidComm")
})
...
Object.defineProperty(credentialRequest, "connection_id", { value: connection_id, enumerable: true })
